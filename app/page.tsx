...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      const t = JSON.parse(
        sessionStorage.getItem('daily-pair-timer') || 'null',
      );
      if (
        t &&
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('daily-pair-timer', JSON.stringify(timer));
    } catch {
  ...
  function begin(kind: Kind) {
    if (timer && timer.kind === kind && timer.day === day) {
      setNow(Date.now());
      setTimer({ ...timer, since: timer.since ? null : Date.now(), elapsed });
    } else {
      if (timer?.since) {
      ...
      setNow(Date.now());
      setTimer({ day, kind, elapsed: 0, since: Date.now() });
    }
  }
  ...
  function calendar() {
    ...
    const lines = [
      ...
      'CALSCALE:GREGORIAN',
      'BEGIN:VEVENT',
      'UID:daily-pair-' + draftStart + '@study',
      'DTSTAMP:' +
        new Date()
          .toISOString()
          .replace(/[-:]/g, '')
